let citations = [
  ...
  let text= document.querySelector('#citation');
  let auteur    = document.querySelector('#auteur');
  let  dernier = 0;
  let nombreAleatoir=0;
  
  button.addEventListener('click', ()=> {
  do {
    nombreAleatoir = Math.round(Math.random()* citations.length)
    console.log (nombreAleatoir)
  } while (dernier==nombreAleatoir);
     
    text.textContent   = citations[nombreAleatoir][0];
    auteur.textContent = citations[nombreAleatoir][1];
    dernier            = nombreAleatoir 
  } );